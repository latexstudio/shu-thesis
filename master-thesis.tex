...
\newcommand{\argmax}{\mathop{\mathrm{argmin}}\limits}
\usepackage{amsfonts}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
这个\LaTeX是我为了写学位论文而写的，虽然写的很简陋，但是完成学位论文写作应该是够了。
...
\begin{figure}[!h]
\centering
\begin{overpic}[scale=0.8]{figures/convergence}
\put(50,38){\large{可使用overpic命令}}
\put(50, 33){\Large{往图像上覆盖符号$\Sigma$}}
\put(50,28){\LARGE{公式$f(x)=a\times x$}}
\put(50,22){\huge{引用公式\ref{eq1}。}}
\put(50,16){\Huge{参考文献\cite{shen2016object}。}}
\end{overpic}
...
\centering
  ...
  \end{tabular}
  \caption{通过表格对图片进行布局，每个表格单元中的图片都是pdf格式的矢量图，
  不论怎么放大都不会失真，生成上面四个图对应的Python代码在\href{https://github.com/zeakey/master-thesis/tree/master/figures}{figures}目录下。}
  \label{fig:fig3d}
\end{figure}
...
所描述的的方法，
以式\ref{eq1}作为评价标准，我们可以得到如图~\ref{fig1}所示的性能曲线以及
